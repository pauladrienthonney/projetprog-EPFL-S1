							 ...
							 int tab_pers[nb_pers][NB_INFOS_PERS])
{
	...
	{
		for(int j=0; j<=NB_INFOS_PERS-1;j++)
		{
					  ...
					  int nb_pers, int tab_pers[nb_pers][NB_INFOS_PERS])
{
	...
	while (condition_arret == 0) 
	{
		...
		for (i = 0 ; i < nb_pers ; i++) 
		{
			...
			{
				contamine_pers_autour(posC, posL, n, nb_pers, tab_pers);
			}
			/* on est arrivé au but, on se déplace. 
			 * A voir si ce test se fait à la fin de la boucle ou au début*/
	...
	//printf("nombre de cycles: %d", cycles);
	return cycles;
}
					 ...
					 int tab_ref[nb_pers][NB_INFOS_PERS])
{
	...
	for(nbP_prime=nb_pers;nbP_prime>=2; nbP_prime--)
	{
		...
		{
				// remplir tab_pers avec les valeurs de départ
